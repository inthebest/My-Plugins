...
public plugin_precache(){
	...
	if(iFile){
		...
		while(fgets(iFile, szBuffer, charsmax(szBuffer))){
			...
			precache_model(fmt("models/%s.mdl", ModelYeri[i]));
			i++;
		}
		ModelSayisi = i;
		fclose(iFile);
...
public RG_CBasePlayerWeapon_DefaultDeploy_(const pEntity, szViewModel[], szWeaponModel[], iAnim, szAnimExt[], skiplocal) {
	if(!(get_member(pEntity, m_iId) != WEAPON_KNIFE)){
		return;
	}

	new id = get_member(pEntity, m_pPlayer);
	SetHookChainArg(2, ATYPE_STRING, ModelAktif[id]);
}
